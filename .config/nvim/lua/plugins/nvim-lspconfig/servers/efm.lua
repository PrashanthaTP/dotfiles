...
local prettier_format_cmd = function(lang)
  ...
  if lang == nil or string.len(lang) == 0 then return command end
end
local python_formatter = {
  formatCommand = "python -m autopep8 --indent-size=4 -",
  formatStdin = true
}
--[===[
...
local lua_formatter = {
  -- formatCommand = FORMATTERS_DIR .. "/stylua -s -",
  formatCommand = FORMATTERS_DIR .. "/lua-format -i",
  formatStdin = true
}
local languages = {
...
M.setup = function(on_attach, capabilities)
  require("lspconfig").efm.setup({
    ...
    filetypes = {
      "css", "html", "javascript", "javascriptreact", "json", "lua", "python",
      "typescript", "typescriptreact", "scss"
    },
